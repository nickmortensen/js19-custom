jQuery(document).ready(($) => {
  ...
  /*
  const CMB2ConditionalsInit = (changeContext, conditionContext ) => {
    let loopI;
    let requiredElements;
    let uniqueFormElements;
    let formElements;
    let bodyElement = document.querySelector('body');
    if ( 'undefined' === typeof changeContext ) {
      changeContext = bodyElement;
    }
    if ( 'undefined' === typeof conditionContext ) {
      conditionContext = bodyElement;
    }
...
*/
  ...
  function CMB2ConditionalsInit(changeContext, conditionContext) {
    ...
    if (typeof conditionContext === 'undefined') {
    ...
    changeContext.on('change', 'input, textarea, select', function (evt) {
      ...
      let dependents;
      const dependentsSeen = [];
      let checkedValues;
      let elmValue;

      // Is there an element which is conditional on this element ?
      dependents = CMB2ConditionalsFindDependants(fieldName, elm, conditionContext);

      // Only continue if we actually have dependents.
      if (dependents.length > 0) {
        ...
        dependents.each((i, e) => {
          let loopIndex = 0;
          const current = $(e);
          const currentFieldName = current.attr('name');
          const requiredValue = current.data('conditional-value');
          const currentParent = current.parents('.cmb-row:first');
          let shouldShow = false;

          // Only check this dependent if we haven't done so before for this parent.
          // We don't need to check ten times for one radio field with ten options,
          // the conditionals are for the field, not the option.
          if (typeof currentFieldName !== 'undefined' && currentFieldName !== '' && $.inArray(currentFieldName, dependentsSeen) < 0) {
            dependentsSeen.push = currentFieldName;

            if (elm.attr('type') === 'checkbox') {
              if (typeof requiredValue === 'undefined') {
                shouldShow = (checkedValues.length > 0);
              } else if (requiredValue === 'off') {
                shouldShow = (checkedValues.length === 0);
            ...
            } else if (typeof requiredValue === 'undefined') {
              shouldShow = (!!elm.val());
            ...
            // If we're hiding the row, hide all dependents (and their dependents).
            if (shouldShow === false) {
              CMB2ConditionalsRecursivelyHideDependants(currentFieldName, current, conditionContext);
            }

            // If we're showing the row, check if any dependents need to become visible.
    ...
    conditionContext.on('CMB2hide', '.cmb-row', function () {
      $(this).children('[data-conditional-required="required"]').each((i, e) => {
    ...
    uniqueFormElms = [];
    $(':input', changeContext).each((i, e) => {
      const elmName = $(e).attr('name');
     ...
     */
    $('.cmb2-wrap > .cmb2-metabox').on('cmb2_add_row', (evt, row) => {
      let rowFormElms;
      const rowRequiredElms = $('[data-conditional-id][required]', row);
      ...
      if (rowFormElms.length === 1 || !rowFormElms.is(':checked')) {
  ...
  function CMB2ConditionalsFindDependants(fieldName, elm, context) {
    ...
    if (typeof (fieldName) !== 'undefined') {
      ...
      if (inGroup.length === 1) {
        ...
        dependents = $('[data-conditional-id]', inGroup).filter(function () {
          const conditionalId = $(this).data('conditional-id');
      ...
      // Else within the whole form.
        ...
        dependents = $(`[data-conditional-id="${fieldName}"]`, context);
      }
    }
    return dependents;
  }
  ...
  function CMB2ConditionalsRecursivelyHideDependants(fieldName, elm, context) {
    let dependents = CMB2ConditionalsFindDependants(fieldName, elm, context);
    dependents = dependents.filter(':visible');

    if (dependents.length > 0) {
      dependents.each((i, e) => {
        const dependent = $(e);
        const dependentName = dependent.attr('name');

        // Hide it.
        dependent.parents('.cmb-row:first').hide();
        if (dependent.data('conditional-required')) {
          dependent.prop('required', false);
        }

        // And do the same for dependents.
        CMB2ConditionalsRecursivelyHideDependants(dependentName, dependent, context);
      });
    }
  }
  ...
  const gutenbergCmb2Wrap = wpWrap.querySelector('.cmb2-wrap');
  switch (true) {
    // for the Classic WordPress Editor.
    case post.length > 0:
      CMB2ConditionalsInit(post, cmb2Wrap);
      break;
    // for the Gutenberg Editor.
    case post.length > 0: // eslint-disable-line no-duplicate-case
      CMB2ConditionalsInit(wpWrap, gutenbergCmb2Wrap);
      break;
    default:
      break;
  }
